...
while menu_option != 0:
    ...
    # Compare Costs, View Plots, Etc.
        ...
        for i in range (0,21):
            T[i] = i
        """
        Use these to prompt the user as to whether they would like to compare costs over different periods of time.
        ...
        for i in range(0,len(inventory)):
            ...
            gas_cost_per_year=(miles_per_year/inventory[i].getMPG())*gas_price
            print ("\n\t" + str(i+1) + ". " + str(inventory[i].getYear()) + " " + str(inventory[i].getMake()) + " " + str(inventory[i].getModel()) + ":")
            print ("\t\tYou will spend about $" + str("{0:.2f}".format(gas_cost_per_year)) + " per year on gas." )
            ...
            """
            NOTE TO SELF: Make sure to add an if statement in the case of the user only wanting a 5 year projection.
            This will prevent the error of trying to access the depreciation at year 10 when it wasn't calculated.
            """
            ...
            for j in range(0,len(T)):
                total_cost = (gas_cost_per_year * j) + Depreciation_Values[j]
        ...
        for i in range(0,len(inventory)):
            ...
            plt.title("Total Cost of Owning a Car Over __ Years")
